...
describe("extractField", () => {
  it("extracts bit 15 when set", () => {
    ...
    const field: FieldDef = { name: "OVER_CHARGED_ALARM", access: "Read", field_type: { Bit: 15 } }
  ...
  it("extracts bit range ERROR_CODE (bits 3–0)", () => {
    ...
    const field: FieldDef = { name: "ERROR_CODE", access: "Read", field_type: { BitRange: { high: 3, low: 0 } } }
    expect(extractField(value, field)).toBe(0xF)
  })

  it("extracts partial bit range", () => {
    const value: RegisterValue = { Word: 0x00A0 }
    const field: FieldDef = { name: "test", access: "Read", field_type: { BitRange: { high: 7, low: 5 } } }
  ...
  it("extracts U16 ByteField from block", () => {
    ...
    const field: FieldDef = { name: "cell1", access: "Read", field_type: { ByteField: { byte_offset: 0, prim: "U16" } } }
  ...
  it("extracts U16 ByteField at offset 2", () => {
    const bytes = new Uint8Array([0x0F, 0xA0, 0x0F, 0xA2])
    const value: RegisterValue = { Block: Array.from(bytes) }
    const field: FieldDef = { name: "cell2", access: "Read", field_type: { ByteField: { byte_offset: 2, prim: "U16" } } }
...
describe("applyField", () => {
  ...
  it("applies BitRange", () => {
    expect(applyField(0x0000, { BitRange: { high: 3, low: 0 } }, 7)).toBe(0x0007)
  })
})