...
export function extractField(
    ...
    if ("Bit" in ft) {
        if (!("Word" in value) && !("SignedWord" in value))
            throw new Error("Bit field requires Word value");
        const word = "Word" in value ? value.Word : value.SignedWord;
        return (word >> ft.Bit) & 1;
    }
    if ("BitRange" in ft) {
        if (!("Word" in value) && !("SignedWord" in value))
            throw new Error("BitRange requires Word value");
        const word = "Word" in value ? value.Word : value.SignedWord;
...
function primitiveStride(prim: string): number {
    switch (prim) {
        ...
        case "I32":
            return 4;
        default:
            throw new Error(`Unknown primitive type: ${prim}`);
    }
}
