...
export function useRegisters() {
  ...
  const writeRegister = useCallback((name: string, value: RegisterValue): Promise<void> => {
    return invoke<void>("write_register", { name, value })
  }, [])

  const writeField = useCallback((register: string, field: string, value: number): Promise<void> => {
