					...
					{registers.map(({ def, value }) => {
						...
						return (
							<Table.Tr key={def.name}>
								...
								<Table.Td>{value ? formatRegisterHex(value) : "—"}</Table.Td>
								{_(REGISTER_SIZE)
									.times()
									.map((_v, i) => {
										const bitIndex = REGISTER_SIZE - i - 1;
										const field = def.fields.find(
											(f) => "Bit" in f.field_type && f.field_type.Bit === bitIndex,
										);
										const isSet = value ? getBit(value, bitIndex) : false;
										return (
											<Table.Td key={`${def.name}-bit-${bitIndex}`}>
												<Center>
													<Button
														variant="outline"
													...
													>
														<Text size="xs">{field ? field.name : String(bitIndex)}</Text>
													</Button>
												</Center>
											</Table.Td>
										);
