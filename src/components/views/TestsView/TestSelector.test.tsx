...
function renderSelector(overrides: Partial<Parameters<typeof TestSelector>[0]> = {}) {
	const props = {
		tests: TESTS,
		selectedId: null,
		onSelect: vi.fn(),
...
describe("TestSelector", () => {
	...
	it("renders the Run button enabled when a test is selected", () => {
		renderSelector({ selectedId: "cell_voltage_balance", isRunning: false });
	...
	it("calls onRun when Run is clicked", () => {
		const onRun = vi.fn();
		renderSelector({ selectedId: "cell_voltage_balance", isRunning: false, onRun });
	...
	it("calls onStop when Stop is clicked", () => {
		const onStop = vi.fn();
		renderSelector({ selectedId: "cell_voltage_balance", isRunning: true, onStop });
