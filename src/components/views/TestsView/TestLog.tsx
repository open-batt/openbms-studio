...
export function TestLog({ logs, onClear }: TestLogProps) {
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: logs triggers re-run for auto-scroll
    useEffect(() => {
        if (viewport.current) {
    ...
    return (
        <Stack h="100%" gap="xs">
            ...
            <ScrollArea
                ...
                styles={{ viewport: { fontFamily: "monospace" } }}
            >
                {logs.length === 0 ? (
                    ...
                    logs.map((entry, i) => (
                        ...
                        >
                            <Group gap="xs" wrap="nowrap">
                                ...
                                <Badge
                                    size="xs"
                                    color={BADGE_COLOR[entry.severity]}
                                    style={{ flexShrink: 0 }}
                                >
                                    {entry.severity}
                                </Badge>
